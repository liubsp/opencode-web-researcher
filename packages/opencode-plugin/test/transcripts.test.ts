...
test("transcripts publish independent Markdown temp copies and regenerate deleted copies", async () => {
  ...
  try {
    ...
    const response = () => ({results:[{local_transcript:{markdown:{path:source,url:"file:///legacy-path.md"},archive_markdown:{path:source}}}]});
    ...
    assert.equal(local.markdown.url, undefined);
