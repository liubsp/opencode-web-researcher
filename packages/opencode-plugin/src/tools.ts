...
export function tools(project: string, call: Call): Info[] {
  const definitions = [
    ...
    { name: "get", description: "Read request state, response/budget or imported-chat metadata. Includes local_transcript.markdown.path: an absolute local file path for reading the captured transcript, not a URL.", input: object({ id }, ["id"]) },
    ...
    { name: "reconcile", description: "After the user fixes login/browser issues, observe an ambiguous submission again without resending. Only works after submission; for a provably unsent failure retry start/send with the same request_key. Only use when explicitly told the issue is fixed.", input: object({ id }, ["id"]) },
